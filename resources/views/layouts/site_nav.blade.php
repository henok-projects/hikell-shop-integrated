...
<nav :class="{'bg-white': !dark, 'bg-gray-900 border-gray-900 md:border-gray-700': dark}"
    class="flex items-center justify-between flex-wrap border-b py-1.5"
    _style="{{ Cookie::get('isDark') ? 'background:#9e9e9e' : 'background:#f0f0f0' }}">
    <div class="relative flex items-center flex-shrink-0 mr-6 text-white lg:px-3">
        <a href="{{ url('site/' . $site->site_name) }}" class="inline-flex items-center  space-x-2">
            @if(file_exists(public_path("/storage/theme/" . $site->site_avatar)) && $curUser->site) 
                <img src="{{ asset('/storage/theme/' . $site->site_avatar) }}" alt="logo" class="object-cover w-8 h-8 mr-1 rounded-full lg:mr-2"/>
            @else
                 <span class="font-semibold rounded-full p-3" :class="{'bg-black text-white': !dark, 'bg-white text-black': dark}">{{ strtoupper(substr($site->site_name,0,1)) }}</span>
            @endif
            <span class="font-semibold" :class="{'text-gray-900': !dark, 'text-white': dark}">{{ $site->site_name }}</span>
        </a>
           
    </div>
    <div class="flex-grow my-2 mr-2 lg:w-auto lg:flex lg:items-center lg:my-0">
        <a href="{{ $promotionLink }}" x-cloak x-data="{tooltip:false}" @click="tooltip=!tooltip"
            @click.away="tooltip = false" role="link" aria-label="tooltip" @mouseover="tooltip=true"
            @mouseout="tooltip=false"
            class="flex relative items-center justify-between block float-right px-2 text-base"
            >
            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current iconSize"
            ...
            @click="tooltip = !tooltip">
                Promote
                @if(auth()->user()->user_id== $site->user_id && !$asUser)
                <div x-show="tooltip" role="tooltip"
                    class="absolute left-0 z-20 w-64 px-4 py-2 -ml-6 transition duration-150 ease-in-out bg-gray-800 border border-gray-700 rounded shadow-lg ">
                    <p class="pb-3 text-xs leading-4 text-white">Based on the plan you are using,You will get 15-33%
                        total commission when some one clicks this and buy our services from here</p>
                </div>
                @endif
            </span>
        </a>
        ...
        <a href="/site" x-cloak x-data="{tooltip:false}" @click="tooltip=!tooltip" @click.away="tooltip = false"
            role="link" aria-label="tooltip" @mouseover="tooltip=true" @mouseout="tooltip=false"
            class="flex relative items-center justify-between block float-right px-2 text-base"
           >
            <svg xmlns="http://www.w3.org/2000/svg" class="fill-current iconSize"
            :class="{'text-black': !dark, 'text-white': dark}"
        ...
        @elseif($curUser->site && $curUser->site->service ==="launch-site")
        <a href="/site" x-cloak x-data="{tooltip:false}" @click="tooltip=!tooltip" @click.away="tooltip = false"
            role="link" aria-label="tooltip" @mouseover="tooltip=true" @mouseout="tooltip=false"
            class="flex relative items-center justify-between block float-right px-2 text-base"
            >
        ...
        @endif
        @else
        <a href="{{ url('/ek-pricing/'). '/'. $site->site_name.  $ext}}" x-cloak x-data="{tooltip:false}"
            @click="tooltip=!tooltip" @click.away="tooltip = false" role="link" aria-label="tooltip"
        ...
        <a href="{{ url('/pricing/'). '/'. $site->site_name.  $ext}}" x-cloak x-data="{tooltip:false}"
            @click="tooltip=!tooltip" @click.away="tooltip = false" role="link" aria-label="tooltip"
            @mouseover="tooltip=true" @mouseout="tooltip=false"
            class="flex relative items-center justify-between block float-right px-2 text-base"
            >
    ...
    <div>
        <a href="{{route('home')}}" class="flex items-centerpx-2 md:pl-44 text-base"
            :class="{'text-black': !dark, 'text-white': dark}">
            Back to Hikel
        </a>
    </div>
    ...
    <div
        ...
        <div class="justify-end flex-grow w-full mr-2 lg:flex lg:items-center lg:w-auto">
            @if(($curUser->site && $curUser->site->service === "launch-site") || ($latestRound && $curUser->enrolled &&
            !$curUser->submittedHGT))
            <x-nav-dropdown>
                <x-slot name="trigger">
                    <button
                ...
                <div class="z-20 py-2 border-b border-l border-r rounded shadow-md w-52 NavDropdownBox"
                    ...
                    @endif
                    @if($latestRound && $curUser->enrolled && !$curUser->submittedHGT)
        ...
        <x-nav-dropdown>
            ...
            <x-slot name="trigger">
                <button
                    ...
                    auth()->user()->avatar)))
                    <img alt="" class="object-cover w-8 h-8 p-1 mr-2 border border-red-900 rounded-full"
            ...
            <div class="z-20 w-48 py-2 mt-3 border-b border-l border-r rounded shadow-md NavDropdownBox"
                ...
                x-init="$watch('isDark', (val) => { $dispatch('switcher', val);  })">
                @auth
                <a class="flex items-center block px-4 py-3 tracking-wide border-b border-gray-700 focus:outline-none"
                href="{{ route('user') }}"
                    :class="{'hover:bg-gray-200 text-dark': !isDark, 'hover:bg-gray-900 hover:text-white text-white': isDark}">
                    ...
                    <span class="ml-2 font-semibold">$ {{ $curUser->balance ? number_format($curUser->balance, 2, '.', '') : '0.00'  }}</span>
                </a>
                <a class="flex items-center block px-4 py-3 tracking-wide border-b border-gray-700 focus:outline-none"
                    href="{{ route('user') }}"
                    :class="{'hover:bg-gray-200 text-dark': !isDark, 'hover:bg-gray-900 hover:text-white text-white': isDark}">
                ...
                {{-- <a class="flex items-center block px-4 py-3 tracking-wide focus:outline-none" href="/settings" :class="{'hover:bg-gray-200 text-dark': !isDark, 'hover:bg-gray-900 hover:text-white text-white': isDark}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="mr-2 fill-current iconSize-bigger" :class="{'text-dark': !isDark, 'text-white': isDark}" viewBox="0 0 24 24">
