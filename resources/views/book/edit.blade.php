...
@section('content')
        ...
        <form action="{{ route('site.book.update',$book->id) }}" method="POST" class="lg:px-10" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex items-center justify-center">
                ...
                <div class="flex flex-col items-center space-y-2">
                    <label class="flex items-center px-4 py-2 tracking-wide uppercase bg-blue-800 border border-gray-500 rounded-lg shadow-lg cursor-pointer text-blue hover:bg-blue-700 hover:text-white">
                        ...
                        <input type="file" name="book_file" class="hidden" value="{{ $book->location }}" accept=".epub,.mobi,.pdf"/>
                    </label>
                </div>
            </div>
            <div class="flex flex-col space-y-3">
                <div class="flex flex-col space-y-2 text-black">
                    ...
                    <x-input  id="title" class="block w-full mt-1"
                     type="text" name="book_title" value="{{ $book->title }}" required autofocus />
                </div>
                <div class="flex flex-col space-y-2 text-black">
                    <x-label class="text-base" for="author" :value="__('Book author')" />
                    <x-input  id="author" class="block w-full mt-1 " name="book_author" required value="{{ $book->author }}" />
                </div>
                ...
                <div class="flex flex-col space-y-2 text-black">
                    ...
                    <x-input placeholder="Book Description" id="description" class="block w-full mt-1 " type="text" name="book_descr" value="{{ $book->description }}" required autofocus />
                </div>
                <div class="flex flex-col space-y-2 text-black">
                    <x-label class="text-base" for="category" :value="__('Book Category')" />
                    <x-textarea placeholder="Book Category" id="categoty" class="block w-full mt-1" name="book_category" required value="{{ $book->category }}" />
                </div>
                <div class="flex flex-col space-y-2 text-black">
                    <x-label class="text-base" for="price" :value="__('Book Price')" />
                    <x-input placeholder="Book Price" id="price" class="block w-full mt-1 " type="number" step="1" min="1" name="book_price" value="{{ $book->price}}" required autofocus />
                </div>
                <div class="flex flex-col space-y-2">
                    ...
                    <div class="flex">
                     ...
                     <input type='file' class="absolute left-0 hidden w-full bottom-3" id="thumbnail" name="book_thumbnail" accept="image/*" value="{{ $book->thumbnail }}"/>
                        </label>
                    </div>
                </div>
                <div class="flex items-center justify-end">
                    <button class="flex items-center justify-between px-3 py-1 mr-4 text-lg text-white bg-red-600 rounded">
                      <a href="/book" ><span>cancel</span></a>
                    </button>
