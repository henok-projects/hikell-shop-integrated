...
@if ($videos->count() > 0)
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 py-2">
        ...
        @php
            $getID3 = new \getID3;
            $video_file = null;
            if(file_exists(public_path("/storage/videos/" . $video->video)))
                $video_file = $video->video ? $getID3->analyze(public_path("/storage/videos/" . $video->video)) : null;
            ...
            $reusedFlag = $video->user_id != $video->original_owner ? '<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-inline fill-current text-red-700"
                                viewBox="0 0 24 24">
        ...
        @endphp
            <div class="flex flex-col text-gray-300 rounded shadow"  :class="{'bg-black hover:bg-gray-800': dark, 'bg-gray-700 hover:bg-gray-800': !dark}" onmouseenter="showPlayIcon(this)" onmouseleave="hidePlayIcon(this)">
                ...
                <div class="flex w-auto h-44 relative cursor-pointer">
                    {{-- thumbnail and duration --}}
                    <img class="object-cover w-full" src="{{ asset("storage/thumbnails/" . $video->thumbnail) }}" alt="">
                    <span class="playCover text-gray-200 bg-red-500 opacity-60 rounded px-1 absolute bottom-2 text-xs right-2">{{ $duration_string }}</span>
                    <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" class="playCover hidden opacity-70 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 w-12 h-12"
                    ...
                    <div class="playCover w-full h-full absolute bg-black opacity-30 hidden"></div>
                </div>
                </a>
                <div class="flex px-3 py-2 flex-col space-y-1">
                    {{-- video details --}}
                    <a href="{{ url('/watch', $video->video_id) }}" class="hover:text-gray-100 hover:font-semibold break-all"><span class="break-words">{{ $video->title }}</span></a>
                    <span class="text-green-600 text-sm">{{ ucfirst($video->owner->username) }}</span>
                    <span class="flex items-center space-x-2 text-xs text-gray-400">
                        <span>{{ $views }} Views</span>  
                        <span>{{ $video->created_at->diffForHumans() }}</span>
                        <span>{!! $reusedFlag !!}</span>
                </div>
            </div>
        @endforeach
    </div>
