...
<div class="table w-full mt-10 py-2 mr-3 bg-white px-5 pt-10">
    {{-- Idol form here --}}
    <h1 class="mb-3 text-2xl font-semibold">Manage <b>{{ ucfirst($idol->name) }}</b> Idol Rounds</h1>
    @if(auth()->user()->admin == 2)
    <form action="{{ route("admin_panel.hgt.round.store", $idol->id) }}" method="POST">
        @csrf
        <input type="hidden" name="idol_id" value="{{ $idol->id }}">
        <div class="flex items-center justify-between px-6 py-5">
            <div class="flex flex-col space-y-2">
                <x-label class="text-base text-gray-900" for="round_name" :value="__('Round Name')" />
                <x-input placeholder="Enter Round Name..." id="round_name" class="block mt-1 w-full" type="text" name="round_name" :value="old('round_name')" required autofocus />
            </div>
            <div class="flex flex-col space-y-2">
    ...
    <table class="w-full space-x-10 border">
        <thead>
            <tr class="bg-gray-50 border-b">
                ...
                <th class="p-2 border-r cursor-pointer text-sm text-gray-800">
        ...
        <tbody>
            {{-- Idol Rounds lists here --}}

            @foreach ($idol->rounds as $item)
